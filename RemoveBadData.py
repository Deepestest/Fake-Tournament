...
def getRatingBasedError(
    ...
    for team in combination:
        totalTrust += scoutRatings["trustRatings"][
            scoutRatings["scouts"].index(combination[team]["metadata"]["scout_info"])
        ]
        data.append(
            {
                ...
                * scoutRatings["trustRatings"][
                    scoutRatings["scouts"].index(
                        combination[team]["metadata"]["scout_info"]
                    )
                ]
    ...
    for game in TBADict:
        ...
        if len(teams) == 3:
            ...
            for i in range(len(combinations)):
                for entry in combinations[i]:
                    scoutTrusts[
                        scouts.index(combinations[i][entry]["metadata"]["scout_info"])
        ...
        for i in range(len(teams)):
            if not len(teamEntries[teams[i]]) == 1:
                ...
                for entry in teamEntries[teams[i]]:
                    ...
                    scoutTrusts[scouts.index(entry["metadata"]["scout_info"])].append(
                        average(entryTrust)
                    )
    for i in range(len(scouts)):
        ...
        for team in teamEntries:
            ...
            for entry in teamEntries[team]:
                if "metadata" in entry.keys():
                    totalTrust += scoutRatings["trustRatings"][
                        scoutRatings["scouts"].index(entry["metadata"]["scout_info"])
                    ]
                    returnEntries[team]["metadata"] = entry["metadata"]
                    for i in range(2):
                        if i == 1:
                            community = "teleop"
                        elif i == 0:
            ...
            for index in indices:
                for team in combinations[index]:
                    combinationScoutError[indices.index(index)] += scoutRatings[
                        ...
                        scoutRatings["scouts"].index(
                            combinations[indices.index(index)][team]["metadata"][
                                "scout_info"
                            ]
                        )
                    ]
...
def getMarkovianRatings(TBAData: pd.DataFrame, scoutingData: list):
            ...
            if len(teams) == 3:
                ...
                for i in range(len(combinations)):
                    for entry in combinations[i]:
                        scoutTrusts[
                            scouts.index(
                                combinations[i][entry]["metadata"]["scout_info"]
                            )
                        ].append(combinationTrust[i])
            # get comparative scout error
            for i in range(len(teams)):
                if not len(teamEntries[teams[i]]) == 1:
                    ...
                    for entry in teamEntries[teams[i]]:
                        flattenedEntry = flatten_dict(entry["data"])
                        totalTrust += oldTrustRatings[scoutRatings["scouts"].index(entry["metadata"]["scout_info"])]
                        for field in flattenedEntry:
                            teamAverage[field] += flattenedEntry[field]*oldTrustRatings[scoutRatings["scouts"].index(entry["metadata"]["scout_info"])]
                    for field in teamAverage:
                        if totalTrust == 0:
                            teamAverage[field] = 0
                        else:
                    ...
                    for entry in teamEntries[teams[i]]:
                        ...
                        scoutTrusts[
                            scouts.index(entry["metadata"]["scout_info"])
    ...
    print(scoutRatings["trustRatings"])
    return scoutRatings
...
def TeamBasedData(TBAData: pd.DataFrame, scoutingData: list) -> list:
    ...
    for team in teams:
        ...
        for entry in teamEntries:
            entryTrust = scoutRatings["trustRatings"][scoutRatings["scouts"].index(entry["metadata"]["scout_info"])]
            for i in range(2):
                if i == 0:
                    communityStr = "auto"
                else:
